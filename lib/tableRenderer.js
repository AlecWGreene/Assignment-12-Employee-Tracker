...
function styleRow(arg_rowData){
    ...
    for(let i = 0; i < fields.length; i++){
        ...
        else if(groupArrays.budget.includes(fields[i])){
            if(arg_rowData[fields[i]]){
                // Calculate expenses/budget ratio
                let t_ratio = arg_rowData["expenses"]/arg_rowData["budget"];

                // Style text based on ratio
                if(t_ratio > 1.1){
                    return_array[fields[i]] = arg_rowData[fields[i]].toString().budgetText_Red;
                }
                else if(t_ratio > 0.85){
                    return_array[fields[i]] = arg_rowData[fields[i]].toString().budgetText_Yellow;
                }
                else{
                    return_array[fields[i]] = arg_rowData[fields[i]].toString().budgetText_Green;
                }
            }
            else{
                return_array[fields[i]] = "";
            }
        }
        else{

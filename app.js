...
colors.setTheme({
    ...
    errorText: ["red", "bold"],
    budgetText_Green: ["green"],
    budgetText_Yellow: ["yellow"],
    budgetText_Red: ["brightRed"]
});
...
/**
 * @function app
 * @description Runs the mainmenu method to init the program
 */
function app(){
    mainMenu();
}
 ...
 * @async 
 * @function mainMenu
 * @description Prompts the user with the options "View Information", "Update Information", "Validate Database", "Exit". The method awaits on calls to the menu functions {@link viewInformationMenu}, {@link updateMenu}, and {@link updateDatabaseIssues}; it will loops until the user selects exit.
*/
 ...
 * @description Presents users with options to view Department, Employee, or Role tables. After selecting an option, the user can use "Select a different table" to reach this prompt again. After the user's selection, this method displays the data using {@link tableRenderer}, prompting the user to alter the view using the menu functions {@link filterMenu}, {@link searchMenu}, {@link sortMenu}, and {@link columnsMenu}. 
 * Function loops through alter view prompts until the user selects "Select a Different table", and then offers the view table prompt until it returns to {@link mainMenu} upon selection of the option "Return to Main Menu".
 */
        ...
        // If the user is deciding how to view the data
            ...
            switch(t_view.method){
                case "Filter Rows":
                    ...
                    t_breadcrumbs[1].filters.push(await filterMenu(t_breadcrumbs[0], t_breadcrumbs[1].columns));
                    if(t_breadcrumbs[1].filters.find( (arg_value) => arg_value.column == "Revert")){
                        t_breadcrumbs[1].filters = [];
                    }
                    break;
                case "Sort Columns":
                    t_breadcrumbs[1].sort = await sortMenu(t_breadcrumbs[0], t_breadcrumbs[1].columns);
                    break;
                case "Search Rows":
                    // Get search criteria
                    let t_search = await searchMenu(t_breadcrumbs[0], t_breadcrumbs[1].columns);
                ...
                case "Select Visible Columns":
                    let t_columns = await columnsMenu(t_breadcrumbs[0], t_breadcrumbs[1].columns);
 ...
 * @async
 * @function filterMenu
 * @param {String} arg_table Name of a table from the database in the format `tableName`_table
 * @description Prompts the user with visible available columns as set by {@link columnsMenu}, where "Revert" will clear the filter. Then upon selection of a column, the user is prompted for an operation and a value to be
 * passed to {@link viewInformationMenu} to assemble into the query.
*/
async function filterMenu(arg_table, arg_columns){
    // Get table fields
    let t_response = await queryDB("SHOW COLUMNS FROM ??;", [arg_table]);
    let t_tableColumns = t_response.map((arg_value) => arg_value.Field);
    if(arg_columns && arg_columns.length > 0){ t_tableColumns = t_tableColumns.filter( arg_value => arg_columns.includes(arg_value)); }
 ...
 * @function searchMenu
 * @param {String} arg_table Name of a table from the database in the format `tableName`_table
 * @description Prompts the user with visible available columns as set by {@link columnsMenu}, then performs a regex search on the column based on the user's input.
 * @todo Implement input validation
*/
async function searchMenu(arg_table, arg_columns){
    // Get table fields
    let t_response = await queryDB("SHOW COLUMNS FROM ??;", [arg_table]);
    let t_tableColumns = t_response.map((arg_value) => arg_value.Field);
    if(arg_columns && arg_columns.length > 0){ t_tableColumns = t_tableColumns.filter( arg_value => arg_columns.includes(arg_value)); }
    ...
    let return_keyword = await prompt([{
        ...
        prefix: "",
        validate: function(arg_input, arg_hash){
            if(arg_input.match(/[a-z\s]/i) && ["name","first_name","last_name","title"].includes(arg_hash.column)){
                return true;
            }
            else if(["name","first_name","last_name","title"].includes(arg_hash.column)){
 ...
 * @function sortMenu
 * @param {String} arg_table Name of a table from the database in the format `tableName`_table
 * @description Prompts the user with visible available columns as set by {@link columnsMenu},
*/
async function sortMenu(arg_table, arg_columns){
    // Get table fields
    let t_response = await queryDB("SHOW COLUMNS FROM ??;", [arg_table]);
    let t_tableColumns = t_response.map((arg_value) => arg_value.Field);
    if(arg_columns && arg_columns.length > 0){t_tableColumns = t_tableColumns.filter( arg_value => arg_columns.includes(arg_value)); }
 ...
 * @param {String} arg_table Name of a table from the database in the format `tableName`_table
*/
async function columnsMenu(arg_table, arg_columns){
    // Get table fields
    let t_response = await queryDB("SHOW COLUMNS FROM ??;", [arg_table]);
    let t_tableColumns = t_response.map((arg_value) => arg_value.Field);
...
async function updateTableMenu(arg_table){
    ...
    while(t_running){
        ...
        let t_input = await prompt({
            type: "list",
            name: "choice",
            message: "How would you like to modify this data?",
    ...
    update: { 
        ...
        employee: [{type: "list", name: "id", message: "Which employee would you like to update?", prefix: "", choices: function(){ // Question -- select an employee
                   ...
                   {type: "list", name: "manager_id", message: "Select a manager for the employee: ", prefix: "", filter: function(arg_input){ // Question -- select a manager_id
                            return arg_input === "Leave Empty" ? 0 : arg_input.match(/\d+(?=\s--)/g);
                        }, choices: function(arg_hash){
                            return new Promise( arg_resolve => {
                                ...
                                queryDB(`(SELECT department_id FROM employee_table WHERE id = ${arg_hash.id})`).then( arg_response2 => {
                                    if(arg_hash.features.includes("department_id")){
                                        ...
                                        t_deparmentId = arg_response2[0].department_id;
                                    }
...
let databaseRules = [
    ...
    // RULE 3 -- Departments cannot have null budgets
    { returnData: [], requirementData: "Department", validator: function(arg_departmentData){
            for(let t_dept of arg_departmentData){
                if(t_dept["budget"] === undefined || t_dept["budget"] === null || t_dept["budget"] === "" || t_dept["budget"] === 0 ){
                    this.returnData.push(t_dept);
                }
            }
        }, message: function(){
            let return_string = "ERROR".errorText + " The following departments do not have a budget: \n";
            for(let i = 0; i < this.returnData.length; i++){
                return_string += this.returnData[i].id + " -- " + this.returnData[i].name + "\n";
            }
            return return_string;
        }
    },
    // RULE 4 -- Roles must have salaries
    { returnData: [], requirementData: "Role", validator: function(arg_roleData){
            for(let t_role of arg_roleData){
                if(t_role["salary"] === undefined || t_role["salary"] === null || t_role["salary"] === "" || t_role["salary"] === 0 ){
                    this.returnData.push(t_role);
                }
            }
        }, message: function(){
            let return_string = "ERROR".errorText + " The following roles do not have a salary: \n";
            for(let i = 0; i < this.returnData.length; i++){
                return_string += this.returnData[i].id + " -- " + this.returnData[i].title;
            }
            return return_string;
        }
    }
...
async function updateDatabaseIssues(){
    ...
    let t_roleData = await queryDB(`SELECT * FROM role_table;`);
    ...
    for(let i = 0; i < databaseRules.length; i++){
        switch(databaseRules[i].requirementData){
            case "Department":
                ...
                break;
            case "Role":
                databaseRules[i].validator(t_roleData);
                break;
            case "Employee":
